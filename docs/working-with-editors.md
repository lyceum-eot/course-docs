...
**NOTE:** Windows users need to install *Windows Subsystem for Linux*, here is the link - _**<a href="https://docs.microsoft.com/en-us/windows/wsl/install-win10" target="_blank">CLICK HERE</a>**_
...
**4.**  After the above step, save the file with the following command that is to be executed inside VIM in the NORMAL mode:
...
**5.** Now, make this bash script *hello.sh* executable:
...
**6.** Finally, we can execute this bash script using the following command:
...
**1.** In first line we're using **curl** command to retrieve data from the API - _<a href="https://corona-stats.online/india" target="_blank">CLICK HERE</a>_, and then stores it in the file **~/.cache/corona**
